...
class ResonanceSingleton {
	 ...
	 */
	static normalize = (pValue: number, pMin: number = 0, pMax: number = 100, pRange: number = 1): number => {
	...
	constructor() {
		...
		if (!this.audioCtx.createGain && (this.audioCtx as any).createGainNode) this.audioCtx.createGain = (this.audioCtx as any).createGainNode;
		/**
		 * Gain node
		 * @type {GainNode}
		 */
	 ...
	 * @returns {ResonanceSingleton} This instance
	 */
	adjustVolume(pVolume: number): ResonanceSingleton {
	 ...
	 */
	addFilter(pSound: Sound, pFilter: any): void {
	 ...
	 */
	removeFilter(pSound: Sound, pFilter: any): void {
		if (pSound instanceof Sound) {
	 ...
	 * @returns {ResonanceSingleton} This instance
	 */
	fade(pVolume: number = 100, pDuration: number = 5000, pEase: string = 'easeOutCubic', pCallback?: () => void): ResonanceSingleton {
		if (isNaN(pVolume)) return this;
		if (isNaN(pDuration)) return this;
		if (this.state === 'fading') return this;
	 ...
	 */
	emit(pSoundPath: string, pVolume: number = 100, pStartTime: number = 0, pEndTime?: number, pPlaybackRate: number = 1, pCallback?: () => void): any {
		...
		// Otherwise load it
			...
			request.onload = function() {
				...
				const error = (pError: any) => {
					self.logger.prefix('Resonance-Module').error(`Error with decoding audio data "${pSoundPath}"`);
				}
	 ...
	 * @returns {ResonanceSingleton} This instance
	 */
	unloadSound(pSoundPath: string): ResonanceSingleton {
	 ...
	 * @returns {ResonanceSingleton} This instance
	 */
	recycleSound(pSound: Sound): ResonanceSingleton {
