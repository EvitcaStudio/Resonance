...
*/
	...
	set volume(pNewVolume) {
		this._volume = ResonanceSingleton.clamp(pNewVolume, ResonanceSingleton.MIN_VOLUME, ResonanceSingleton.MAX_VOLUME);
		if (this.loaded) this.gainNode.gain.value = ResonanceSingleton.normalize(this._volume);
	}
    /**
	...
	build(pSoundPath, pVolume=100, pStartTime=0, pEndTime=0, pSave=false, pPlayUnfocused=false, pPlaybackRate=1, pLoop=false) {
		...
		this._volume = ResonanceSingleton.clamp(pVolume, ResonanceSingleton.MIN_VOLUME, ResonanceSingleton.MAX_VOLUME);
