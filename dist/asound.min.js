...
f={};VYLO.global.aSound=f;window.aSound=f;window.AudioContext=window.AudioContext||window.webkitAudioContext;f.audioCtx=new AudioContext;f.audioCtx.createGain||(f.audioCtx.createGain=f.audioCtx.createGainNode);f.gainNode=f.audioCtx.createGain();f.gainNode.gain.value=m(200);f.gainNode.connect(f.audioCtx.destination);f.soundsPlaying=[];f.suspendedSounds=[];f.recycledSounds=[];f.queuedSoundsToPlay=[];f.queuedSoundsToFade=[];f.loadedBuffers={};f.fader={};f.volume=100;f.muted=!1;f.state=null;f.focused=
!0;f.canPlaySound=function(){return"function"!==typeof VYLO.Client.canPlaySound||VYLO.Client.canPlaySound()?!0:!1};f.adjustVolume=function(a){this.volume=Math.max(-200,Math.min(a,200));this.gainNode.gain.value=m(this.volume)};f.fade=function(a,b=5,c="easeOutCubic",d){if(!isNaN(a)&&!isNaN(b)&&"fading"!==this.state){a=Math.max(-200,Math.min(a,200));r.includes(c)||(c="easeOutCubic",console.warn("aSound['fade'] Invalid pEase value. Reverted to default."));this.state="fading";this.fader.duration=Math.max(b,
...
typeof k.onSuspended&&(this.onSuspended=k.onSuspended.bind(this)),"function"===typeof k.onResumed&&(this.onResumed=k.onResumed.bind(this))):k&&k.constructor!==Object&&console.warn("aSound Library: Invalid variable type passed for pCallbackObject.");f.loadedBuffers[this.soundName]?this.loaded=!0:this.load()}load(){const a=new XMLHttpRequest,b=this;a.open("GET",VYLO.Resource.getResourcePath("sound",b.soundName));a.responseType="arraybuffer";a.onload=function(){f.audioCtx.decodeAudioData(a.response,
...
this.fader.queue.duration,this.fader.queue.ease,this.fader.queue.callback)}}var p=()=>{"running"!==f.audioCtx.state&&f.audioCtx.resume().then(()=>{},()=>{});f.ready=!0};window.addEventListener("mousedown",p);window.addEventListener("touchstart",p);AListener.addEventListener(VYLO.Client,"onWindowBlur",function(){f.focused=!1;f.soundsPlaying.length&&f.suspendAllSounds(!0)});AListener.addEventListener(VYLO.Client,"onWindowFocus",function(){f.focused=!0;f.suspendedSounds.length&&f.resumeAllSounds(!0);
(f.queuedSoundsToPlay.length||f.queuedSoundsToFade.length)&&f.playQueuedSounds()});AListener.addEventListener(VYLO.Client,"onDisconnect",function(){f.stopAllSounds()});AListener.addEventListener(VYLO.Client,"onNew",function(){p()});AListener.addEventListener(VYLO.Client,"onDel",function(){f.stopAllSounds()})}else console.error("Your device does not support window.AudioContext || window.webkitAudioContext. This library cannot be used")}})();